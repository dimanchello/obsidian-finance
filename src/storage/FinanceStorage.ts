...
export interface AccountRecordsFile {
  version: number;
...
export function addToSet(arr: string[], value: string): void {
  const v = value.trim();
  if (v && !arr.includes(v)) arr.push(v);
}

export function rebuildLookups(file: AccountRecordsFile): void {
  file.categories = [];
  file.tags = [];
  file.payers = [];
...
export class FinanceStorage {
  ...
  async load(accountId: string): Promise<AccountData> {
    // Parallel loading (~6× faster than sequential)
    const [meta, recs, debts, credits, deposits, exchanges] = await Promise.all([
      this.loadMeta(accountId),
      this.loadRecords(accountId),
      this.loadDebts(accountId),
      this.loadCredits(accountId),
      this.loadDeposits(accountId),
      this.loadExchanges(accountId),
    ]);
  ...
  async deleteDebt(accountId: string, id: string): Promise<void> {
    await this.loadDebts(accountId); // Ensure cache is populated
    this.debts.deleteById(accountId, id);
  }

  async deleteDebtsBatch(accountId: string, ids: string[]): Promise<void> {
    await this.loadDebts(accountId); // Ensure cache is populated
    this.debts.deleteBatch(accountId, ids);
  }
  ...
  async deleteCredit(accountId: string, id: string): Promise<void> {
    await this.loadCredits(accountId); // Ensure cache is populated
    this.credits.deleteById(accountId, id);
  }

  async deleteCreditsBatch(accountId: string, ids: string[]): Promise<void> {
    await this.loadCredits(accountId); // Ensure cache is populated
    this.credits.deleteBatch(accountId, ids);
  }
  ...
  async deleteDeposit(accountId: string, id: string): Promise<void> {
    await this.loadDeposits(accountId); // Ensure cache is populated
    this.deposits.deleteById(accountId, id);
  }

  async deleteDepositsBatch(accountId: string, ids: string[]): Promise<void> {
    await this.loadDeposits(accountId); // Ensure cache is populated
    this.deposits.deleteBatch(accountId, ids);
  }
  ...
  async deleteExchange(accountId: string, id: string): Promise<void> {
    await this.loadExchanges(accountId); // Ensure cache is populated
    this.exchanges.deleteById(accountId, id);
  }

  async deleteExchangesBatch(accountId: string, ids: string[]): Promise<void> {
    await this.loadExchanges(accountId); // Ensure cache is populated
    this.exchanges.deleteBatch(accountId, ids);
  }
  ...
  async deleteDebtsWithLinkedRecords(accountId: string, debtIds: string[]): Promise<void> {
    ...
    const data = await this.loadRecords(accountId);
    const filtered = data.records.filter(r => !r.linkedId || !debtIdSet.has(r.linkedId));
  ...
  async deleteCreditsWithLinkedRecords(accountId: string, creditIds: string[]): Promise<void> {
    // Load only credits before deletion (not full account data)
    const creditsBefore = await this.loadCredits(accountId);
    const creditIdSet = new Set(creditIds);
    const dpIds = new Set(creditsBefore.filter(c => creditIdSet.has(c.id) && c.downPaymentRecordId).map(c => c.downPaymentRecordId!));

    await this.deleteCreditsBatch(accountId, creditIds);

    // Load only records after deletion (not full account data)
    const recordsAfter = await this.loadRecords(accountId);
    const filtered = recordsAfter.records.filter(r => (!r.linkedId || !creditIdSet.has(r.linkedId)) && !dpIds.has(r.id));
    await this.saveAllRecords(accountId, filtered);
  }
  ...
  async deleteDepositsWithLinkedRecords(accountId: string, depositIds: string[]): Promise<void> {
    ...
    // Use Set for O(N) filtering instead of O(N×M) with includes
    const depositIdSet = new Set(depositIds);
    const data = await this.loadRecords(accountId);
    const filtered = data.records.filter(r => !r.linkedId || !depositIdSet.has(r.linkedId));
    await this.saveAllRecords(accountId, filtered);
  }
