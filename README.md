...
### Вариант 2: Через BRAT (рекомендуется для бета-версий)

1. Установите плагин **BRAT** (Beta Reviewers Auto-update Tool):
   - Настройки → Сторонние плагины → Отключить безопасный режим
   - Нажмите "Обзор" → найдите "BRAT" → установите
2. Включите BRAT в настройках
3. Вызовите команду: `BRAT: Add a beta plugin for testing`
4. Введите репозиторий: `https://github.com/dimanchello/obsidian-finance`
5. Подтвердите → плагин установится автоматически
...
- **Внутренние операции**: кнопка 🔄 в поле «Плательщик» помечает запись как внутреннюю — она не учитывается в статистике (доходы/расходы/баланс). Фильтр «Внутренние» в панели фильтров позволяет показать только такие записи.
